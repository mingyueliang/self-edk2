...
class IfrTree:
        ...
        try:
            with open(FileName, "w") as f:
                ...
                if self.PreProcessDB.VfrDict != {}:
                    Index = 0
                    for Key in self.PreProcessDB.VfrDict:
            ...
            if Root.OpCode is not None:
                ...
                if Root.OpCode == EFI_IFR_VARSTORE_OP:
                    ...
                    if Root.Data.HasVarStoreId:
                        if "varid" in Root.Dict:
                            ...
                            f.write(
                                f"{ValueIndent}varid:  {'0x%04x' % Info.VarStoreId} # Optional Input\n"
                            )
                ...
                if Root.OpCode == EFI_IFR_VARSTORE_EFI_OP:
                    ...
                    if Root.Data.HasVarStoreId:
                        if "varid" in Root.Dict:
                            f.write(
                                f"{ValueIndent}varid:  {Root.Dict['varid'].Key} # Optional Input\n"
                            )
                        else:
                            f.write(
                                f"{ValueIndent}varid:  {'0x%04x' % Info.VarStoreId} # Optional Input\n"
                    ...
                    if Root.Data.NameItemList != []:
                        ...
                        for NameItem in Root.Data.NameItemList:
                            f.write(
                                ...
                                + " - name:  "
                                + "STRING_TOKEN("
                                + self.PreProcessDB.GetKey(NameItem)
                                + ")\n"
                            )
                ...
                if Root.OpCode == EFI_IFR_FORM_OP:
                    ...
                    if "formid" in Root.Dict:
                        f.write(f"{ValueIndent}formid:  {Root.Dict['formid'].Key}\n")
                    else:
                ...
                if Root.OpCode == EFI_IFR_SUBTITLE_OP:
                    ...
                    f.write(
                        f"{ValueIndent}text:  STRING_TOKEN({self.PreProcessDB.GetKey(Info.Statement.Prompt)})\n"
                    )
                ...
                if Root.OpCode == EFI_IFR_NUMERIC_OP:
                    ...
                    self._DumpQuestionInfosWithUni(Root, f, ValueIndent)
                    if "max" in Root.Dict:
                        f.write(f"{ValueIndent}maximum:  {Root.Dict['max'].Key}\n")
                    else:
                        f.write(f"{ValueIndent}maximum:  {'0x%0x' % Info.Data.MaxValue}\n")

                    if "min" in Root.Dict:
                        f.write(f"{ValueIndent}minimum:  {Root.Dict['min'].Key}\n")
                    else:
                ...
                if Root.OpCode == EFI_IFR_STRING_OP:
                    ...
                    self._DumpQuestionInfosWithUni(Root, f, ValueIndent)
                    if "minsize" in Root.Dict:
                        f.write(f"{ValueIndent}minsize:  {Root.Dict['minsize'].Key}\n")
                    else:
                        f.write(f"{ValueIndent}minsize:  {Info.MinSize}\n")

                    if "maxsize" in Root.Dict:
                        f.write(f"{ValueIndent}maxsize:  {Root.Dict['maxsize'].Key}\n")
                    else:
                ...
                if Root.OpCode == EFI_IFR_REFRESH_OP:
                    ...
                    if Root.Condition is not None:
                        f.write(f"{ValueIndent}condition:  '{Root.Condition}'\n")
                    if "interval" in Root.Dict:
                        f.write(ValueIndent + "interval:  " + Root.Dict["interval"].Key + "\n")
                    else:
                        f.write(
                            f"{ValueIndent}interval:  {Info.RefreshInterval}  # RefreshInterval\n"
                        )
    ...
    def DumpBuffer(self, Root):
        Buffer = ""
        for i in range(0, len(Root.Buffer)):
            Buffer += f"{Root.Buffer[i]:0>2X} "
        Buffer = f"'< {Root.Offset:0>8X}: {Buffer}>'"
        return Buffer
