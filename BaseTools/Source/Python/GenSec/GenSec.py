...
def main():
    ...
    while argc > 0:
        ...
        try:
            if sys.argv[arg_index] == "-s" or sys.argv[arg_index] == "--SectionType":
                ...
                if not SectionName or sys.argv[arg_index+1].startswith("-"):
            ...
            if sys.argv[arg_index] == "-o" or sys.argv[arg_index] == "--outputfile":
                ...
                if not OutputFileName or sys.argv[arg_index+1].startswith("-"):
            ...
            if sys.argv[arg_index] == "-c" or sys.argv[arg_index] == "--compress":
                ...
                if not CompressionName or sys.argv[arg_index+1].startswith("-"):
            ...
            if sys.argv[arg_index] == "-g" or sys.argv[arg_index] == "--vendor":
                try:
                    VendorGuid = ModifyGuidFormat(sys.argv[arg_index+1])
            ...
            if sys.argv[arg_index] == "--dummy":
                ...
                if not DummyFileName or sys.argv[arg_index+1].startswith("-"):
            ...
            if sys.argv[arg_index] == "-r" or sys.argv[arg_index] == "--attributes":
                if not sys.argv[arg_index + 1] or sys.argv[arg_index+1].startswith("-"):
            ...
            if sys.argv[arg_index] == "-l" or sys.argv[arg_index] == "--HeaderLength":
                ...
                if EFI_ERROR(Status):
                    logger.error("Invalid option value for GuidHeaderLength, %s = %s" % (sys.argv[arg_index], sys.argv[arg_index+1]))
                    raise
                argc -= 2
                arg_index += 2
                continue
            ...
            if sys.argv[arg_index] == "-n" or sys.argv[arg_index] == "--name":
                ...
                if not StringBuffer or sys.argv[arg_index+1].startswith("-"):
            ...
            if sys.argv[arg_index] == "d" or sys.argv[arg_index] == "--debug":
                res = AsciiStringToUint64(sys.argv[arg_index + 1], False, LogLevel)
                if isinstance(res, int):
                    Status = res
                else:
                    Status = res[0]
            ...
            if sys.argv[arg_index] == "--sectionalign":
                ...
                if sys.argv[arg_index + 1] == "0":
                    ...
                    if EFI_ERROR(Status):
                        logger.error("Invalid option value:Alignment = %s" % sys.argv[arg_index+1])
                        raise
                argc -= 2
                arg_index += 2
                InputFileAlignNum += 1
            ...
            # No value given for the end parameter
            logger.error("Invaild option value, %s option" % sys.argv[arg_index])
            return STATUS_ERROR
        ...
        InputFileName.append(sys.argv[arg_index])
        InputFileNum += 1
        argc -= 1
        arg_index += 1
    ...
    if DummyFileName:
        ...
        try:
            ...
            logger.error("Error opening file: %s" % DummyFileName)
            return STATUS_ERROR
    ...
    if SectType != EFI_SECTION_GUID_DEFINED and SectType != EFI_SECTION_FREEFORM_SUBTYPE_GUID and \
            SectionName != None and (GUID().__cmp__(VendorGuid)):
    ...
    if SectType == EFI_SECTION_FREEFORM_SUBTYPE_GUID and (GUID().__cmp__(VendorGuid)):
    ...
    if SectType == EFI_SECTION_COMPRESSION: # TODO: EFICOMPRESS Issue
        res = GenSectionCompressionSection(InputFileNum, SectCompSubType, InputFileName, InputFileAlign)
        if isinstance(res, int):
            Status = res
        else:
            Status = res[0]
        ...
        VersionSect = Get_VERSION_Header(nums)()
        CommonHeader.Type = SectType
        CommonHeader.SET_SECTION_SIZE(Index)

        VersionSect.BuildNumber = VersionNumber

        Enc = StringBuffer.encode()
        # VersionSect.VersionString.__setitem__(nums, 0)
        for i in range(nums):
            ch = Enc[i]
            VersionSect.VersionString[i] = ch

        OutFileBuffer = struct2stream(CommonHeader) + struct2stream(VersionSect) + b'\0\0'
        ...
        CommonHeader = EFI_COMMON_SECTION_HEADER()
        UiSect = Get_USER_INTERFACE_Header(nums)()
        CommonHeader.Type = SectType
        CommonHeader.SET_SECTION_SIZE(Index)
        ...
        OutFileBuffer = struct2stream(CommonHeader)+ struct2stream(UiSect) + b'\0\0'
